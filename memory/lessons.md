...
### 2026-07-10 (weekly review) — From Aggressive Bull: pyramiding into an existing, confirming winner is a productive use of idle cash — Cautious Bull has no equivalent rule
AGGRO's only trade this week was a disciplined pyramid add — 15 more VST shares (its only green position, +4.476% pre-market) bought July 10, funded from cash, stop immediately reissued to cover the full position, explicitly reasoned as "never averaging down — an add to a winner." Meanwhile Cautious Bull sat on 80.343% cash through a week SPY rallied +1.35%, with both held positions (LLY +8.855%, V +7.838%) clean, confirming winners well below their 20% single-position caps. Cautious Bull's strategy.md has no scale-up rule for existing winners — only for new entries. Proposed (not yet adopted): a winner that has cleared +15% unrealized gain, has an intact thesis, and is below the 20% cap becomes eligible for a small (2-4% of portfolio) add funded from excess cash, under the same guardrails as a new entry. Flagged for the next strategy revision rather than implemented unilaterally off one week's observation. Cross-Bull counter: AGGRO trails Cautious by ~6.2pp since AGGRO's own June 4 inception (narrowed from ~9.5pp as AGGRO's concentrated book caught this week's rally) — NOT triggered (needs AGGRO ahead by >5pp for 2 consecutive reviews).

### 2026-07-10 (weekly review) — Six consecutive diligenced names failed only the technical-extension gate, never fundamentals
LRCX, ETN, CEG, VRT, NVT, and ABBV were all diligenced between mid-June and July 10; every one passed earnings/catalyst/valuation (signals #1, #2, #3, #5) and failed only signal #4 (technical confirmation / not >10% extended above the 50-day MA). This is not six unrelated near-misses — it is a structural pattern: names that screen well fundamentally have usually already rallied into extended territory by the time they clear diligence, because the same catalyst that makes them fundamentally attractive is often what already moved the stock. Re-running full diligence on the same handful of names every few sessions, only to hit the identical technical gate each time, produced zero new positions across 5+ weeks. Response: added a "pullback-watch" protocol to strategy.md — log a pullback target price (the 50-day MA) once at diligence time instead of re-diligencing from scratch, and auto-promote when price reaches it with the other signals still intact. Also codified a 0.5%-minimum-separation rule for the "above the 50-day MA" test itself, since ETN sat in an undefined noise-level-crossing limbo for two sessions with no written threshold to resolve it — an unwritten, inconsistently-applied judgment call is itself a discipline gap under the strategy's own "do not invent gates beyond what's written" principle.
...
### 2026-07-06 (close) — A diversified, non-tech book lags hard on tech/semi-led rally days — the flip side of the cash/diversification shield
Bull's since-inception gap vs SPY widened from −0.930pp (pre-market) to −2.007pp (close) in a single session, even though Bull's own return was only mildly negative (−0.205%). The entire swing came from SPY: it rallied +0.95% today (Nasdaq +1.2%, Dow past 53,000) on a broad AI/semi-trade rebound, while LLY (healthcare) and V (financials) — neither tech-adjacent — slipped −0.78% and −1.38% respectively with no negative company news. This is the mirror image of the well-documented cash-drag/diversification protection on down days (June 5, June 9, June 23-26): a portfolio deliberately built outside the hottest-momentum sector will structurally lag on days that sector snaps back hard, exactly as it will outperform when that sector sells off. Neither reaction is a mistake — it is the expected behavior of the diversification choice already made. No rule change; noted so a future review doesn't mistake a sector-rotation day for thesis decay in LLY or V.

### 2026-07-03 (weekly review) — From Aggressive Bull: an approved plan is not executed until a routine actually places it
AGGRO's July 2 pre-market plan called for a 25% proactive AVGO trim (9 shares) at market open — all conditions met, fully reasoned. But the market-open routine never placed the order (or didn't run). By midday, AVGO's buffer had compressed from 2.92pp to 0.54pp before the trim was finally completed, at a materially worse price than intended. AGGRO's own lesson: "a fully-approved pre-market trade is not real until a routine actually places it — every downstream routine must verify the prior routine's plan actually fired, not just assume it did." This applies equally to Cautious Bull's own pre-market → market-open handoff. **Rule change proposed:** before a market-open or midday routine journals "0 trades" / "plan was empty," it must first check whether the *prior* routine's `Planned trades` JSON block was non-empty, and if so, confirm a matching fill exists in Alpaca's order/position history — not simply assume the plan already executed or silently carry it forward. This is a process-verification fix, not a risk-appetite change; Cautious Bull's diversification and 10% stops remain correct per all prior comparisons.
...
### 2026-06-25 (midday) — NVDA II: GPU compute spot pricing is a real-time leading indicator of AI infrastructure demand health
NVDA's second stop-out (−9.78% in 3 days, June 22–25) was driven in part by GPU compute lease price compression: Nvidia's B200 GPU hourly spot market price fell from $6.11 (May 30) to $4.22 (June 21), a −31% decline in 3 weeks. This reflects rising hardware competition and expanding cloud compute supply, directly threatening GPU premium pricing power. The original thesis assumed AI accelerator monopoly pricing remained intact. GPU spot prices are publicly available in real time (CoreWeave/Lambda spot markets) and provide earlier warning than quarterly earnings. For any future AI infrastructure holding, track GPU spot pricing as a secondary ongoing thesis health metric alongside fundamental catalysts. When GPU spot falls >20% in a month, treat as a thesis watch-flag.

### 2026-06-24 (close) — NVDA: when a stated invalidation condition triggers at close, the next pre-market is the decision point — do not wait for the trailing stop
NVDA's entry plan stated "closes below $200 on volume" as the invalidation condition. At 15:51 ET close it printed $197.75 (−7.34% from entry), breaching both the $200 invalidation level and the −7% forced-cut threshold ($198.48). The trailing stop (at $192.591) had NOT fired. Because the close routine cannot trade, the position stayed open. The lesson: when a named invalidation condition triggers at EOD, log it immediately in the close journal and mandate a hold/exit decision at pre-market the next session. A thesis break is categorically different from a stop-out — it means the original reason for holding is gone, and the stop price is no longer the primary exit signal. Waiting for the stop to fire when the thesis is already broken turns a deliberate strategy position into a passive one.
