...
- Staggered 10 min after Cautious Bull's pre-market to reduce ordinary overlap;
  this is not a cross-host lock and does not prevent a collision or late run.
  The routine UI has no timezone picker — crons run in UTC.
...
command. Today's fenced plan must have exactly top-level `schema_version: 1`,
