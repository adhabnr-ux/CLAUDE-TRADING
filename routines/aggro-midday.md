...
command. A verified `midday_loss` is the sole same-day buy/sell risk-reduction
exception and must target the entire live holding for a new operation. Recovery
of a known partial fill must reuse its immutable original target so the gateway
submits only the cumulative remainder. Planned trims/exits cannot day-trade and
remain blocked after a same-day buy.

Then read memory/aggressive/profile.md and every file in memory/aggressive/.
Follow the playbook in .claude/commands/aggro-midday.md exactly.
